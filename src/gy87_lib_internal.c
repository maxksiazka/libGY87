...
uint8_t write_reg(gy87_t* dev, uint32_t dev_addr, uint8_t reg_addr,
    ...
    payload[1] = reg_val;
    int ret =
        dev->config->i2c_write(dev->config->ctx, dev_addr, payload, 2, false);
    return ret;
}
...
//     uint8_t count = *reg_addr_val_pair_list++; // count is the first element
//     in the list int32_t bytes_sent = 0; while (count) {
//         int32_t ret = dev->i2c_ops->i2c_write(dev->i2c_ops->user_ctx,
//         dev->addr, reg_addr_val_pair_list, 2,
//                                 false);
//         if (ret != 2) {
